...
- `Bot::on_decision_latency(latency_ms: u64)` trait method. Fires after
...
- Non-default `User-Agent` header on the WebSocket handshake:
  `chipzen-sdk-rust/<version>`. Defense-in-depth alongside the
  platform-side Cloudflare path allowlist so the default
  `tokio-tungstenite` UA doesn't trip the bot-fight rule on the
  `chipzen.ai` zone. (#46)
- Public `SDK_VERSION` constant and `user_agent()` function so
