...
- **`Bot::on_reconnected(&mut self, msg)` lifecycle hook.** Fired when the
  server resumes an in-flight match with a `reconnected` frame (which is
  what a re-attach delivers instead of `match_start`), so a bot can
  (re)learn its match context — `match_id`, `seats`, `game_config` —
  before the replayed pending turn is decided. Default impl is a no-op;
  existing bots are unaffected.
  ([#121](https://github.com/chipzen-ai/chipzen-sdk/issues/121))

### Fixed

- **`your_seat` is re-learned from `reconnected.seats` before the pending
  ...
  with `your_seat = 0` regardless of the bot's actual seat. The
  `reconnected` handler now reads the `is_self` seat from
  `reconnected.seats` — the same schema as `match_start.seats`
  (TRANSPORT-PROTOCOL §8.15) — mirroring the Python SDK fix for
  chipzen-ai/chipzen-sdk#119.
  ([#121](https://github.com/chipzen-ai/chipzen-sdk/issues/121))
