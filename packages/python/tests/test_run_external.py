...
class _CapturingWS:
    """Mock lobby websocket recording everything sent. Returns an ``evict``
    frame on the first ``recv()`` so ``run_external_bot``'s lobby loop returns
    promptly (status ``"evicted"`` -> no reconnect) after the authenticate
    frame is sent."""
...
class _CapturingConnect:
    ...
    Accepts the lobby-connect kwargs ``run_external_bot`` passes (``max_size``,
    ``user_agent_header``, and ``subprotocols`` on the gateway leg)."""
    ...
    def __call__(self, url, *, max_size=None, user_agent_header=None, subprotocols=None):
...
def _install_connect_stub(monkeypatch) -> _CapturingConnect:
    ...
    # run_external_bot calls the top-level ``websockets.connect`` (see
    # chipzen.external), so patch that exact attribute — patching the
    # ``websockets.asyncio.client`` submodule's connect would leave the
    # top-level reference untouched and let the test hit a real network.
    import websockets
...
class TestRunExternalCli:
    ...
    resolve connection, and call ``run_external_bot`` against the stub WS
    server."""
    ...
    def test_runs_against_stub_server(self, tmp_path, monkeypatch):
        ...
        ``run_external_bot`` which connects to the stubbed lobby websocket
        and sends an authenticate frame."""
        _bypass_config_discovery(monkeypatch, tmp_path)
        _clear_chipzen_env(monkeypatch)
